...
-- (ConcertCosts project). Safe to run more than once.

-- ========== Migration 002: currency + ticket groups ==========
...
create policy "Users view own ticket groups"
  on public.concert_ticket_groups for select to authenticated
...
create policy "Users insert own ticket groups"
  on public.concert_ticket_groups for insert to authenticated
...
create policy "Users update own ticket groups"
  on public.concert_ticket_groups for update to authenticated
  using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "Users delete own ticket groups"
  on public.concert_ticket_groups for delete to authenticated
  using (auth.uid() = user_id);
...
-- Tell Supabase API to refresh table list (helps clear "schema cache" errors)
